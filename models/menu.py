...
response.title = myconf.get('app.name')
tmp_domain = myconf.get('app.domain')
response.logo = A(B(SPAN(response.title.split()[0]),
                    ' ',
                    LABEL(response.title.split()[1])),
                  _class="navbar-brand",
                  _id="web2py-logo",
                  _href=tmp_domain)
response.subtitle = ''
...
response.meta.author_no_email = myconf.get('app.author_no_email')
